<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:77:"D:\www\twothink\public/../application/admin/view/default/community\index.html";i:1511936292;s:73:"D:\www\twothink\public/../application/admin/view/default/public\base.html";i:1496373782;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
...
<div class="data-table table-striped">
    <table>
        ...
        <tbody>
        <?php if(!(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty()))): if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$community): $mod = ($i % 2 );++$i;?>
        <tr>
            <td><input class="ids row-selected" type="checkbox" name="" id="" value="<?php echo $community['id']; ?>"> </td>
            <td><?php echo $community['title']; ?></td>
            <td><?php echo $community['content']; ?></td>
            <td><?php echo $community['admin']; ?></td>
            <td><?=date('Y-m-d h:i:s',$community['create_time']);?></td>
            <td><?php echo $community['click']; ?></td>
            <td><?=$community['status']==1?'发布':'未发布'?></td>
            <td>
                <a title="编辑" href="<?php echo url('edit?id='.$community['id'].'&pid='.$pid); ?>">编辑</a>
                <a title="状态" href="<?php echo url('status?id='.$community['id'].'&pid='.$pid); ?>"><?=$community['status']==1?'未发布':'发布'?></a>
                <a class="confirm ajax-get" title="删除" href="<?php echo url('del?id='.$community['id']); ?>">删除</a>
            </td>
        </tr>
